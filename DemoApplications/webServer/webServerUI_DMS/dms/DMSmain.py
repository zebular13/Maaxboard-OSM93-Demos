...
# Run vela models
# MODEL_PATH = pathlib.Path("/home/root/webServerUI_DMS/vela_models")
...
DELEGATE_PATH = pathlib.Path("/usr/lib/libethosu_delegate.so")
DELEGATE_PATH = None


# Run non-vela models
#MODEL_PATH = pathlib.Path("./SimpleDMS/models")
#MODEL_PATH = pathlib.Path("./models")

MODEL_PATH = pathlib.Path("/home/root/webServerUI_DMS/models")
DETECT_MODEL = "face_detection_front_128_full_integer_quant_vela.tflite"
LANDMARK_MODEL = "face_landmark_192_integer_quant_vela.tflite"
EYE_MODEL = "iris_landmark_quant_vela.tflite"
...
def InitDMSmain():
    ...
    print(str(MODEL_PATH / DETECT_MODEL))
    print(DELEGATE_PATH)
    ...
    cap = cv2.VideoCapture(cap_input, cv2.CAP_V4L)
    ret, image = cap.read()
    if not ret:
    ...
    face_detector = FaceDetector(model_path = str(MODEL_PATH / DETECT_MODEL), delegate_path=DELEGATE_PATH, img_size = (target_dim, target_dim))
    face_mesher = FaceMesher(model_path=str(MODEL_PATH / LANDMARK_MODEL), delegate_path=DELEGATE_PATH)
    eye_mesher = EyeMesher(model_path=str(MODEL_PATH / EYE_MODEL), delegate_path=DELEGATE_PATH)
